...
articlesRouter.get("/", cacheMiddleware(), async (c) => {
...
articlesRouter.get("/search", cacheMiddleware(), async (c) => {
...
articlesRouter.get("/related", cacheMiddleware(), async (c) => {
...
articlesRouter.get("/:slug/key-finding", cacheMiddleware(), async (c) => {
  const article = await getArticleBySlug(c.req.param("slug"));
  if (!article) return c.json({ error: "Not found" }, 404);
  return c.json({ text: article.keyFinding ?? "" });
});

articlesRouter.get("/:slug/sources", cacheMiddleware(), async (c) => {
  const article = await getArticleBySlug(c.req.param("slug"));
  if (!article) return c.json({ error: "Not found" }, 404);
  return c.json(article.sources ?? []);
});

articlesRouter.post("/:slug/views", async (c) => {
...
articlesRouter.get("/:slug/views", cacheMiddleware(30), async (c) => {
  const article = await getArticleBySlug(c.req.param("slug"));
  if (!article) return c.json({ error: "Not found" }, 404);
  return c.json({
    articleId: article._id.toString(),
...
articlesRouter.get("/:slug", cacheMiddleware(), async (c) => {
  const article = await getArticleBySlug(c.req.param("slug"));
  if (!article) {
...
articlesRouter.post("/", publicSubmissionRateLimitMiddleware, validateBody(articleSubmitSchema), async (c) => {
  const body = c.get("validatedBody") as z.infer<typeof articleSubmitSchema>;
